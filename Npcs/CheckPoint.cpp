...
void CheckPoint::update(sf::Time frametime)
{
	...
	if (m_isValidated)
	{
		...
		if (m_timer > m_timerMax)
			m_timer = sf::Time::Zero;
		m_builder.clear();
		createEffect(m_size, getPosition() + sf::Vector2f(15.f, -190.f), std::pow(m_timer / m_timerMax, 0.867f), m_color, m_builder);
		createEffect(m_size, getPosition() + sf::Vector2f(15.f, -190.f), std::pow(m_timer / m_timerMax, 0.12f), m_color, m_builder);
