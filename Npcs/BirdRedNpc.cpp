...
void BirdRedNpc::setupMachine(void)
{
...
void BirdRedNpc::setPosition(sf::Vector2f const & position)
{
...
void BirdRedNpc::updateState(void)
{
	octo::CharacterSprite & sprite = getSprite();
...
void BirdRedNpc::updatePhysics(void)
{
	octo::CharacterSprite & sprite = getSprite();
	if (sprite.getCurrentEvent() == Special1)
	{
...
void BirdRedNpc::collideOctoEvent(CharacterOcto * octo)
{
	ANpc::collideOctoEvent(octo);
}

void BirdRedNpc::draw(sf::RenderTarget & render, sf::RenderStates states) const
{
	if (!m_animationEnd)
