...
WaterCascadeSystem::WaterCascadeSystem() :
	m_engine(std::time(0)),
	m_creationTimeDistri(0.1f, 0.2f),
	m_lifeTimeDistri(1.6f, 1.8f),
	m_directionDistri(0.f, octo::Pi),
	m_timer(sf::Time::Zero),
	m_nextCreation(sf::Time::Zero),
	m_color(sf::Color(28, 172, 228, 110)),
	m_canEmit(false)
{
	static float const				Size = 2.f;
	WaterCascadeSystem::Prototype	prototype;

	prototype.emplace_back(sf::Vertex({Size, Size}));
	prototype.emplace_back(sf::Vertex({Size, -Size}));
	prototype.emplace_back(sf::Vertex({-Size, -Size}));
	prototype.emplace_back(sf::Vertex({-Size, Size}));
	reset(prototype, sf::Quads, 2000);
}

void	WaterCascadeSystem::canEmit(bool canEmit)
{
	m_canEmit = canEmit;
}

void	WaterCascadeSystem::setColor(sf::Color const & color)
{
	m_color = color;
}

void	WaterCascadeSystem::setEmitter(sf::Vector2f const & emitter)
{
	m_emitter = emitter;
}

void	WaterCascadeSystem::update(sf::Time frameTime)
{
...
void	WaterCascadeSystem::updateParticle(sf::Time frameTime, Particle& particle)
{
	static float const			Velocity = 30.f;
...
bool	WaterCascadeSystem::isDeadParticle(Particle const& particle)
{
