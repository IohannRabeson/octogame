...
void	LevelZeroScreen::update(sf::Time frameTime)
{
	...
	if (m_state == Flying)
	{
		...
		m_spaceShip.setSmokeVelocity(sf::Vector2f(octo::linearInterpolation(-1400.f, -200.f, interpolateValue) * frameTime.asSeconds(), 0.f));
	...
	else if (m_state == Falling)
	{
		...
		m_spaceShip.setSmokeVelocity(sf::Vector2f(-200.f * frameTime.asSeconds(), octo::linearInterpolation(-20.f, -400.f, interpolateValue) * frameTime.asSeconds()));
		createBackground(sf::Vector2f(cameraRect.left, cameraRect.top), color);
